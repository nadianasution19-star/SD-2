...
// TODO : Hapus node terakhir 
    void deleteLast() {
    if (head == NULL){
    cout << "liast nya kosong\n";
    return;
    }

    if(head == tail){ // kalo ada 1 node
        delete head;
        head = tail = NULL;
        return;
    }
...
// TODO : Hapus node dengan nilai tertentu 
    ...
    Node *p = head;
    while (p -> next != NULL && p -> next -> value != value){ // letakkan p sblm sesuatu yg dihapus
    p = p -> next;
}
...
int main() { // Contoh list awal insertLast(10); insertLast(20); insertLast(30);
    cout << "List awal:\n";
    printList();
